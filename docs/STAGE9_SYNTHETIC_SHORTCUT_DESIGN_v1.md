...
- **전처리 정합성 (2026-08-13 갱신 — 오버라이드 가능해짐)**: 원래 `TorchvisionAdapter`는 `weights=None`이어도 항상 `weights_enum.DEFAULT.transforms()`(squeezenet1_0 기준 Resize(256)→CenterCrop(224)→ImageNet Normalize)를 오디트 시점 전처리로 썼고, 이 값은 설정으로 오버라이드할 수 없었다. 이후 `TorchvisionAdapter`에 `preprocessing_ops` 인자가, `TorchvisionProviderConfig`에 `preprocessing` 필드가 추가되어(`ssat/core/adapter/torchvision_adapter.py`, `ssat/core/adapter/provider.py`) 이제는 `ssat.core.adapter.preprocessing.DeclarativePreprocessor`를 통해 임의의 선언적 전처리(예: crop 없는 `Resize([224,224])`)를 지정할 수 있다. 학습 스크립트(`train.py`, `evaluate_accuracy.py`)도 `--preprocessing {preset,crop_free}` 플래그로 같은 선택지를 제공하며, `crop_free`는 어댑터와 동일한 `DeclarativePreprocessor` 클래스를 그대로 구동해(수치적으로 비슷한 것이 아니라 같은 코드 경로로) 학습/감사 시점 전처리 불일치를 방지한다(`common.py`의 `build_crop_free_transform`). 원래의 CenterCrop 프리셋을 감사한 원 L3 실행이 이 제약으로 인해 겪은 모델 공간 면적 아티팩트와 그 crop-free 재실행 결과는 `docs/L3_Synthetic-Shortcut Experiment Report.md`와 `docs/CONTROL_STABILITY_DESIGN_v1.md` §0 addendum을 참고.
