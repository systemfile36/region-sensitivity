...
## 7. Phase 7 반영 현황 (2026-08-24 추가)

Phase 6이 회귀 위험 때문에 보류했던 §4 항목들을 마무리하는 별도 리팩터링 패스에서, 기본 동작을 변경하지 않는 선에서 §4의 남은 항목 대부분을 처리했다. 검증은 일반 테스트(`docker compose exec region-sensitivity-workspace bash -lc 'pytest -q'`, 전체 1093개 통과)로 각 항목마다 반복 수행했고, 실제 추론 경로를 건드리는 항목(어댑터 공통화)과 `AuditApplication` 파사드 분할에 대해서는 추가로 `experiments/synthetic_shortcut/reproduce_demo.sh`(CIFAR-10 기반 재현 데모)를 end-to-end로 실행해 Q1-Q5 사전 등록 판정이 모두 PASS로 재현됨을 확인했다(벤치마크 결과물은 `.gitignore`된 스크래치 디렉터리에만 쓰이므로 기존에 커밋된 벤치마크 산출물은 그대로 유지된다).

이번 패스에서는 `ssat/report/`의 **구조**(§2.2의 `assets.py` R3 위반, `ReportDataAssembler`의 통계 직접 계산, `AuditApplication.generate_report()`의 미분해 등)를 전혀 건드리지 않았다 — 사용자 요청에 따라 의도적으로 현재 구조를 유지하고 이 문서에 한계점으로만 기록한다. `generate_report()`는 시그니처와 본문 모두 `ssat/application/application.py`에 그대로 남아 있다.

- **처리됨 — §4 항목 2 (`ModelLoader`/`ModelRunner` 공통 계약 추출)**: `ssat/core/adapter/_torch_adapter_mixin.py`의 `_TorchAdapterMixin`에 device 결정(`_place_on_device`)·OOM cleanup(`cleanup_after_oom`)·seed 초기화(`_validate_init_seed`/`_seeded_model_init`)·output decoder 검증(`_finalize_output_decoder`)·`predict()`의 3단계 예외 래핑(`_wrap_stage`/`_wrap_inference`)을 추출해 `TorchvisionAdapter`/`TorchvisionVideoAdapter`/`TorchvisionTSMAdapter`/`TimmAdapter` 4곳에 믹스인으로 적용했다. `TorchvisionTSMAdapter.predict()`가 preprocess/inference를 하나로 묶고 decode를 감싸지 않는 기존의 비일관성은 `_wrap_inference`의 `passthrough_adapter_error` 플래그로 정확히 재현해 그대로 보존했다(의도적으로 "고치지" 않음). 2-way로만 중복되던 `_resolve_weights`도 `_torch_helpers.resolve_torchvision_weights`로 추출했다. 회귀 테스트: `tests/unit/test_model_adapter.py`, `tests/unit/test_torchvision_tsm_adapter.py`, `tests/unit/test_timm_geometry_mode.py`, `tests/unit/test_adapter_provider.py`, `tests/unit/test_report_adapters.py`, `tests/integration/test_runtime_e2e.py` — 그리고 `experiments/synthetic_shortcut/reproduce_demo.sh`의 end-to-end 실행(Q1-Q5 전부 PASS).
- **처리됨 — §4 항목 3 (`StrategyRegistry[T]` 제네릭화)**: `ssat/core/_strategy_registry.py`에 `register()`의 타입 검사·중복 등록 거부 로직만 제네릭 `StrategyRegistry[T]`로 추출했다. `RegionMaskGeneratorFactory`/`RegionFamilyExpanderFactory`/`OperatorFactory`는 내부적으로 이를 위임할 뿐, 각자의 클래스 이름과 `build()`/`build_operators()`의 시그니처 비대칭(context 유무)은 그대로 유지해 공개 API를 전혀 바꾸지 않았다. `AdapterProviderRegistry`/`SourceProviderRegistry`는 `parse()`/`build()`가 Pydantic 판별에 결합되어 있어 이번 통합 대상에서 의도적으로 제외했다. 회귀 테스트: `tests/unit/test_region_mask_factory.py`, `tests/unit/test_region_expander_factory.py`, `tests/unit/test_perturb_operator_factory.py`, `tests/unit/test_region_expander.py`.
- **처리됨 — §4 항목 4 (Provider 이름 일치의 정적 검증)**: `ssat/core/_provider_registry_support.py`의 `validate_config_literal()`이 `AdapterProviderRegistry.register()`/`SourceProviderRegistry.register()`에서 `config_model`의 판별 필드(`provider`/`kind`)가 `provider.name`과 실제로 일치하는지 등록 시점에 검사한다. `Literal["x"] = "x"` 형태뿐 아니라 사용자 확장이 흔히 쓰는 `str = "x"` 기본값 형태도 통과하도록, `Literal`의 허용값 집합과 필드 기본값을 함께 검사하는 방식으로 설계했다(순수 `Literal` 일치만 요구했다면 `tests/unit/test_source_provider.py`의 커스텀 provider 테스트가 깨졌을 것). 현재 내장 8쌍(adapter 4 + source 4) 모두 이 검사를 통과한다. 회귀 테스트: `tests/unit/test_adapter_provider.py`, `tests/unit/test_source_provider.py`.
- **처리됨 — §4 항목 6 (`WorkItem.identity_payload()` 단일화)**: `ssat/core/plan/types.py`에 `_item_identity_payload()`를 추가해 `WorkItem.identity_payload()`와 `PlanBuilder._make_item`(`ssat/core/plan/builder.py`)이 동일한 7-키 딕셔너리 구성을 공유하도록 했다. 딕셔너리 키 순서·내용은 이전과 완전히 동일해 `item_id` 해시에 영향이 없다. 회귀 테스트: `tests/unit/test_runtime_types.py`.
- **부분 처리 — §4 항목 7 (`region_id::region_instance_id` 키 포맷 / `_check_region_geometry` 단일 소스화)**: 실제 코드(`ssat/analysis/indexer.py`의 모듈 docstring과 306-308행 인라인 주석)를 직접 확인한 결과, `ssat.analysis` 패키지는 `ssat.metrics`뿐 아니라 `ssat.core` 자체에도 의존하지 않기로 이미 명시적으로 결정되어 있었다(`strategy_profile.py`의 `PerturbationOp` 참조 하나가 유일한 예외). §4 항목 7이 제안한 `ssat/core` 하위 공유 추출 방향은 이 의도적 설계 결정과 배치되므로, **패키지 내부** 중복만 제거했다: `ssat/metrics/types.py`에 metrics 쪽(`aggregate.py`/`viz/mask_check.py`) 전용 `region_key()`를, `ssat/analysis/types.py`에 analysis 쪽(`indexer.py`/`interval.py`/`control.py`/`stability.py`/`reader.py`) 전용 `region_key()`/`region_key_column()`을 각각 독립적으로 두었다. metrics↔analysis 경계를 넘는 통합과 `_check_region_geometry`의 크로스 패키지 통합은 아래처럼 의도적으로 보류했다. 회귀 테스트: `tests/unit/test_metrics_aggregate.py`, `tests/unit/test_metrics_mask_check.py`, `tests/unit/test_analysis_indexer.py`, `tests/unit/test_analysis_interval.py`, `tests/unit/test_analysis_control.py`, `tests/unit/test_analysis_stability.py`, `tests/unit/test_analysis_reader.py`.
- **의도적으로 보류 — `_check_region_geometry`의 metrics/analysis 통합**: 유일한 중복이 metrics↔analysis 경계를 넘는 것이며, 그 경계 회피가 `ssat/analysis/indexer.py:306-308`에 코드 주석으로 명시적으로 문서화되어 있어("this module intentionally stays off `ssat.core`"), 이를 통합하려면 이미 의도적으로 거부된 의존을 새로 만들어야 한다. `ssat/report/`의 구조적 한계를 문서화만 하고 보류한 것과 동일한 판단으로, 여기서도 코드 변경 없이 이 사실만 기록해 둔다.
- **처리됨 — §4 항목 1 (`AuditApplication` 분할)**: `generate_report()`를 제외한 모든 공개 메서드(`prepare_run`/`execute_run`/`estimate`/`inspect`/`compute_metrics`/`analyze`/`export_labels`/`rebuild_index`)의 본문을 `ssat/application/_session_service.py`(공유 세션 플러밍: `build_context`/`load_skeleton_store`/`resume_for_context`/`output_mode`/`preflight_fingerprint`/`output_snapshot`/`emit`)·`_run_service.py`·`_estimate_service.py`·`_metrics_service.py`·`_analysis_service.py`·`_labels_service.py`·`_dump_service.py`로 옮기고, `AuditApplication`은 순수 위임(facade delegation)으로 남겼다. 공개 시그니처·`__init__` 시그니처·관측 가능한 동작·독스트링은 바이트 단위로 동일하게 유지했고, `generate_report()`는 시그니처와 본문 모두 손대지 않았다(같은 파일, 같은 위치, 재포맷도 하지 않음). `PreparedRun`/`_ExecutionContext`도 계속 애플리케이션 계층의 공개 표면에 남아 있다(`_ExecutionContext`만 `_session_service.py`로 옮기고 `application.py`가 그 정의를 재사용). 회귀 테스트: `tests/integration/test_application_api.py`(18개 테스트, 가장 핵심적인 안전망), `tests/integration/test_cli.py`, `tests/integration/test_video_source_e2e.py`, `tests/integration/test_skeleton_application_e2e.py`, `tests/unit/test_adapter_provider.py` — 그리고 `experiments/synthetic_shortcut/reproduce_demo.sh`의 end-to-end 실행(`run_audit.py`가 이 파사드를 통해 실제 추론을 수행, Q1-Q5 전부 PASS).
- **추가로 발견·처리됨 (리포트 콘텐츠, §4 범위 밖)**: `ssat/report/{html_renderer,adapters,assembler}.py`의 한국어 문자열(템플릿·매크로·포맷 헬퍼·`MetricCard.note` 약 150여 곳)을 전부 영어로 번역했고, `<html lang="ko">`도 `lang="en"`으로 맞췄다. `ssat/report/`의 **구조**는 사용자 요청에 따라 전혀 변경하지 않았다(§2.2의 `assets.py` R3 위반, `AuditApplication.generate_report()` 미분해 등은 계속 보류). 리터럴 콘텐츠 교체에 맞춰 `tests/unit/test_report_html_renderer.py`(~25개 assertion)·`tests/integration/test_report_synthetic_dump.py`·`tests/unit/test_report_adapters.py`의 하드코딩된 한국어 assertion도 함께 갱신했고, `tests/unit/test_report_labels.py`/`test_report_assembler.py`/`test_report_types.py`의 테스트 전용 fixture·docstring도 일관성을 위해 함께 번역했다(`tests/unit/test_utils.py`의 한국어는 `write_json_atomic`의 무관한 유니코드 왕복 테스트 데이터라 손대지 않았다). 회귀 테스트: 위에서 언급한 리포트 관련 테스트 전체.
- **추가로 발견·처리됨 (CSS, §4 범위 밖)**: `ssat/report/static.py`의 `.run-meta` 그리드에 `min-width: 0`(`.run-meta > div`)과 `overflow-wrap: break-word`(`.run-meta dd`)를 추가해, 긴 `model_id`/`preprocessing_desc` 값이 이웃 컬럼(예: 전처리 블록)을 침범하던 `report.html`의 레이아웃 버그를 수정했다.

`ssat/report/`의 나머지 구조적 한계(§2.2의 `assets.py` R3 위반, `ReportDataAssembler`의 통계 직접 계산, `AuditApplication.generate_report()`의 미분해, §3.5 항목 10의 `html_renderer.py` 카드 키 하드코딩 — §4 항목 9)는 이번 패스에서도 사용자 요청에 따라 **의도적으로 손대지 않았다** — 별도 후속 작업으로 남겨둔다.
